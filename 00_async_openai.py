...
class Config:
    ...
    OPENAI_API_VERSION: str = "2023-10-01"
...
class AsyncOpenAILanguageModel(AsyncLanguageModel):
    """Async implementation for OpenAI language model."""
    ...
    def __init__(self, model: str, api_key: Optional[str] = None, **kwargs):
        ...
        # Validate model name (can be expanded based on available models)
        valid_models = {'gpt-4', 'gpt-3.5-turbo', 'gpt-4-turbo-preview'}  # Add more as needed
        if not any(valid_model in model for valid_model in valid_models):
            logging.warning(f"Model '{model}' may not be a valid OpenAI model name")
    ...
    async def ainvoke(self, prompt: str) -> str:
        try:
            ...
            # Update with kwargs, but don't override the essential parameters
            kwargs_filtered = {k: v for k, v in self.kwargs.items() 
                            if k not in ['model', 'messages', 'response_format']}
            config.update(kwargs_filtered)
    ...
    async def aclose(self):
        # OpenAI's API doesn't require explicit closing
        pass
...
class TestHarness:
    ...
    def _create_prompt(self, question: str, options: QuestionOptions) -> str:
        ...
        options_text = "\n".join(
            f"{letter.value}. {option.strip()}" 
            for letter, option in options.options
        )
        
        return (
            f"Question: {question.strip()}\n\n"
            f"Options:\n{options_text}\n\n"
            f"Select the best answer from the options above. "
            f"Respond ONLY with 'Answer: X' where X is the letter of your chosen answer."
        )
    ...
    async def evaluate_question(
        ...
        try:
            ...
            if retry_count < Config.MAX_RETRIES:
                ...
                retry_delay = Config.RETRY_DELAY * (2 ** retry_count)
            ...
            *Progress.get_default_columns(),
            ...
            for csv_file in csv_files:
                ...
                try:
                    ...
                    questions = [
                        (row, int(row['id'])) for _, row in df.iterrows()
                    ]
...
async def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="o1-preview",
        help="Model to use for evaluation (default: o1-preview)"
    )
    parser.add_argument(
        "--api-key",
        type=str,
        default=None,
        help="OpenAI API key (default: taken from OPENAI_API_KEY environment variable)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Delay between batches to respect rate limits (default: {Config.RATE_LIMIT_DELAY} seconds)"
    )
    parser.add_argument(
        "--request-timeout",
        type=float,
        default=Config.REQUEST_TIMEOUT,
        help=f"Request timeout for API calls (default: {Config.REQUEST_TIMEOUT} seconds)"
    )
    parser.add_argument(
        "--retry-delay",
        type=float,
        default=Config.RETRY_DELAY,
        help=f"Delay before retrying after a failure (default: {Config.RETRY_DELAY} seconds)"
    )
    ...
    model = AsyncOpenAILanguageModel(
        model=args.model,
        api_key=args.api_key,
        temperature=1  # This will be part of kwargs
    )
